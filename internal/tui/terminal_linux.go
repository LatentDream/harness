...
func platformTerminalState(fd uintptr) (any, error) {
	...
	if err := terminalIOCTL(fd, syscall.TCGETS, unsafe.Pointer(&state)); err != nil {
		return nil, err
	}
	return state, nil
}
...
func platformSetRaw(fd uintptr, original any) error {
	...
	return terminalIOCTL(fd, syscall.TCSETS, unsafe.Pointer(&state))
}
...
func platformWaitReadable(fd uintptr, timeout time.Duration) (bool, error) {
	...
	set.Bits[index] |= 1 << (fd % 64)
	timeval := syscall.NsecToTimeval(timeout.Nanoseconds())
	count, err := syscall.Select(int(fd)+1, &set, nil, nil, &timeval)
